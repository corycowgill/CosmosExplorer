...
export class HUD {
  ...
  updateTargetArrows(camera, aliens, playerPos) {
    ...
    for (const { a, d } of cands) {
      ...
      el.style.color = a.type === 'boss' ? '#ff5ec4' : (a.type === 'warden' ? '#ff3355' : (a.type === 'cruiser' ? '#ffaa33' : (a.type === 'fighter' ? '#ff66cc' : (a.type === 'sentinel' ? '#33ddff' : (a.type === 'stinger' ? '#ff5522' : (a.type === 'lancer' ? '#ffee44' : '#66ff88'))))));
  ...
  drawRadar(player, aliens) {
    ...
    for (const a of aliens) {
      ...
      const onRim = mag > 1;
      const boss = a.type === 'boss' || a.type === 'warden';
      ctx.fillStyle = a.type === 'boss' ? '#ff5ec4' : (a.type === 'warden' ? '#ff3355' : (a.type === 'cruiser' ? '#ffaa33' : (a.type === 'fighter' ? '#ff66cc' : (a.type === 'sentinel' ? '#33ddff' : (a.type === 'stinger' ? '#ff5522' : (a.type === 'lancer' ? '#ffee44' : '#66ff88'))))));
      ctx.beginPath();
